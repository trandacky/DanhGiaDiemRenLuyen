...
import com.qnu.cnttk40a.config.KafkaProperties;
...
class PhieuRenLuyenKafkaResourceIT {
    ...
    void setup() {
        ...
        PhieuRenLuyenKafkaResource kafkaResource = new PhieuRenLuyenKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/phieu-ren-luyen-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/phieu-ren-luyen-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
